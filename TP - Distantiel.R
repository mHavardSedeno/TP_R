...
#1. 
texte <- scan(file = "cyrano.txt", what = "character", n = 40)
...
FREQ <- matrix(0, nrow = length(freq), ncol = length(freq));FREQ
...
for (i in 1:length(freq)){
  for (j in 1:length(freq)){
    if (i != j){
      ...
      FREQ[i,j] = FreqMotSuivant1(names(freq[i]),names(freq[j]))
    }
  }
};FREQ
